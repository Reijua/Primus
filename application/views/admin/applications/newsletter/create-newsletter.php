<div class="modal-content create-post" id="modal-replace">
	<div class="modal-header">
		...
		<h4 class="modal-title">Newsletter erstellen</h4>
	</div>
			...
			<div class="row">
				<div class="col-lg-6">
					...
					<div class="form-group">
						...
						<textarea class="form-control" rows="12" id="text" name="text" placeholder="Text"><?= set_value('text') ?></textarea>
						<?= form_error('text'); ?>
					</div>
					<div class="form-group">
						<label>Empfänger</label><br />
						<label for="member">
							<input type="checkbox" name="recipient" value="member" id="member"> Member (<?= $memberC['COUNT(*)'] ?>)&nbsp;&nbsp;&nbsp;
						</label>
						<label for="partner">
							<input type="checkbox" name="recipient" value="partner" id="partner"> Partner (<?= $partnerC['COUNT(*)'] ?>)&nbsp;&nbsp;&nbsp;
						</label>
						<label for="employee">
							<input type="checkbox" name="recipient" value="employee" id="employee"> Employee (<?= $employeeC['COUNT(*)'] ?>)
						</label>
					</div>
				</div>
				<div class="col-lg-6">
...
$(document).ready(function() {
	...
	function savePost() {
		...
		$.ajax({
			type: 'POST',
			url: '/admin/newsletter/create-newsletter',
			data: data,
			contentType: false,
			processData: false,
