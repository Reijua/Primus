<div class="modal-content create-post" id="modal-replace">
	<div class="modal-header">
		...
		<h4 class="modal-title">Event erstellen</h4>
	</div>
			...
			<div class="row">
				<div class="col-lg-6">
					...
					<div class="form-group">
						...
						<textarea class="form-control" rows="12" id="description" name="description" placeholder="Beschreibung"><?= set_value('description') ?></textarea>
						<?= form_error('description'); ?>
					</div>
					<div class="form-group">
						<label for="startdate">Beginn</label>
						<input type="text" class="form-control" id="startdate" name="startdate" placeholder="Beginn" value="<?= set_value('startdate') ?>">
						<?= form_error('startdate') ?>
					</div>
					<div class="form-group">
						<label for="enddate">Ende</label>
						<input type="text" class="form-control" id="enddate" name="enddate" placeholder="Ende" value="<?= set_value('enddate') ?>">
						<?= form_error('enddate') ?>
					</div>
				</div>
				<div class="col-lg-6">
					...
					<div class="form-group">
						...
						<select class="form-control" id="leader" name="leader">
							...
							<option value="<?= $row->member_id ?>" <?= set_select('leader', $row->member_id) ?>><?= $row->member_firstname.' '.$row->member_lastname ?></option>
							<?php endforeach; ?>
						</select>
						<?= form_error('leader'); ?>
					</div>
					<div class="form-group">
						...
						<select class="form-control" id="eventtype" name="eventtype">
							...
							<option value="<?= $row->eventtype_id ?>" <?= set_select('eventtype', $row->eventtype_id) ?>><?= $row->eventtype_name ?></option>
							<?php endforeach; ?>
						</select>
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" id="maxmember" name="maxmember" placeholder="Teilnehmeranzahl" value="<?= set_value('maxmember') ?>">
...
$(document).ready(function() {
	...
	$(function() {
		...
		// Dirty fix for wrong datetimepicker positioning
