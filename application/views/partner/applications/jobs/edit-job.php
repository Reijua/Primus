...
<div class="modal-content" id="modal-replace">
	<div class="modal-header">
		...
		<h4 class="modal-title">Jobangebot bearbeiten</h4>
	</div>
			...
			<div class="row">
				<div class="col-lg-6">
					...
					<div class="form-group">
						...
						<textarea class="form-control" rows="4" id="lead-text" name="lead-text" placeholder="Vorwort"><?= empty(form_error('lead-text')) ? $lead_text : set_value('lead-text') ?></textarea>
						<?= form_error('lead-text'); ?>
					</div>
					<div class="form-group">
						<label for="text">Text</label>
						<textarea class="form-control" rows="8" id="text" name="text" placeholder="Text"><?= empty(form_error('text')) ? $text : set_value('text') ?></textarea>
						<?= form_error('text'); ?>
					</div>
				</div>
				<div class="col-lg-6">
					<div class="form-group">
						...
						<textarea class="form-control" rows="4" id="salary-text" name="salary-text" placeholder="Gehaltsangabe"><?= empty(form_error('salary-text')) ? $salary_text : set_value('salary-text') ?></textarea>
					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<select class="form-control" id="status" name="status">
									<option value="0" <?= (empty(form_error('status')) ? ($status == 0 ? 'selected' : '') : set_select('status', 0)) ?>>Geschlossen</option>
									<option value="1" <?= (empty(form_error('status')) ? ($status == 1 ? 'selected' : '') : set_select('status', 1, true)) ?>>Offen</option>
								</select>
								<?= form_error('status'); ?>
							</div>
						</div>
						<div class="col-md-6">
							<div class="form-group">
								...
								<input type="text" class="form-control" id="start-date" name="start-date" placeholder="Eintrittsdatum" value="<?= empty(form_error('start-date')) ? $start_date : set_value('start-date') ?>">
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<select class="form-control" id="contact" name="contact">
									...
									<option value="<?= $row->employee_id ?>" <?= (empty(form_error('contact')) ? ($contact == $row->employee_id ? 'selected' : '') : set_select('contact', $row->employee_id)) ?>><?= $row->employee_title ?> <?= $row->employee_firstname ?> <?= $row->employee_lastname ?></option>
									<?php endforeach; ?>
								</select>
								<?= form_error('contact'); ?>
							</div>
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<select class="form-control" id="type" name="type">
									...
									<option value="<?= $row->type_id ?>" <?= (empty(form_error('type')) ? ($type == $row->type_id ? 'selected' : '') : set_select('type', $row->type_id)) ?>><?= $row->type_name ?></option>
									<?php endforeach; ?>
								</select>
								<?= form_error('type'); ?>
							</div>
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<select class="form-control" id="location" name="location">
									...
									<option value="<?= $row->location_id ?>" <?= (empty(form_error('location')) ? ($location == $row->location_id ? 'selected' : '') : set_select('location', $row->location_id)) ?>><?= $row->location_name ?> (<?= $row->address_city ?>, <?= $row->country_name ?>)</option>
									<?php endforeach; ?>
								</select>
								<?= form_error('location'); ?>
							</div>
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<select class="form-control" id="sector" name="sector">
									...
									<option value="<?= $row->sector_id ?>" <?= (empty(form_error('sector')) ? ($sector == $row->sector_id ? 'selected' : '') : set_select('sector', $row->sector_id)) ?>><?= $row->sector_name ?></option>
									<?php endforeach; ?>
								</select>
								<?= form_error('sector'); ?>
							</div>
