<div class="modal-content create-contact" id="modal-replace">
	<div class="modal-header">
		...
		<h4 class="modal-title">Kontaktperson hinzufügen</h4>
	</div>
			...
			<div class="row">
				<div class="col-lg-6">
						...
						<div class="col-sm-6">
							<div class="form-group">
								...
								<select class="form-control" id="salutation" name="salutation">
									...
									<option value="female" <?= set_select('salutation', 'female'); ?>>Frau</option>
									<option value="male" <?= set_select('salutation', 'male'); ?>>Herr</option>
								</select>
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" id="firstname" name="firstname" placeholder="Vorname" value="<?= set_value('firstname') ?>">
						<?= form_error('firstname') ?>
					</div>
					<div class="form-group">
						<label for="lastname">Nachname</label>
						<input type="text" class="form-control" id="lastname" name="lastname" placeholder="Nachname" value="<?= set_value('lastname') ?>">
						<?= form_error('lastname') ?>
					</div>
					<div class="form-group">
						<label for="email">E-Mail</label>
						<input type="email" class="form-control" id="email" name="email" placeholder="E-Mail-Adresse" value="<?= set_value('email') ?>">
						<?= form_error('email') ?>
					</div>
					<div class="form-group">
						<label for="phone">Telefonnummer</label>
						<input type="text" class="form-control" id="phone" name="phone" placeholder="Telefonnummer" value="<?= set_value('phone') ?>">
						<?= form_error('phone') ?>
					</div>
				</div>
				<div class="col-lg-6">
...
$(document).ready(function() {
	...
	function saveContact() {
		...
		$.ajax({
			type: 'POST',
			url: '/partner/contacts/create-contact',
			data: data,
			contentType: false,
			processData: false,
