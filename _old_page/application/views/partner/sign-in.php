...
<div class="banner">
	...
	<div class="banner-content">
		<?php
		foreach ($this->P_account_model->get_account("all")->result() as $row) {
		echo '<div class="logo-item">
				<table>
					<tr>
						<td><img src="'.$this->P_account_model->get_logo($cdn_url, $row->company_id).'" alt="'.$row->company_name.'" /></td>
					</tr>
				</table>
			  </div>';
		}
...
<?php }else{ ?>
<div class="banner" style="background-image: url(<?=$this->P_account_model->get_banner($cdn_url, $this->input->cookie("partner_id")) ?>)">
	<table class="partner-logo">
		<tr>
			<td><img src="<?=$this->P_account_model->get_logo($cdn_url, $this->input->cookie("partner_id")) ?>"></td>
		</tr>
	</table>
</div>
<?php } ?>
			...
			<div class="column-content">
				...
				<form methode="post" data-url="/ajax/account/signin/" data-redirect="/" data-type="normal">
					...
					<table style="width:100%; border-collapse:collapse; border-spacing:0; margin-bottom:10px;">
						<tr>
							<td class="text-left"><input type="checkbox" name="email_save" id="signin_email_save" value="1"<?=($this->input->cookie("partner_id") == NULL ? ' ' : ' checked="checked"')?>/><label for="signin_email_save" style="font-weight:300;">E-Mail-Adresse merken</label></td>
