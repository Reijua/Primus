...
class M_account_model extends CI_Model {
    ...
    function create_account($salutation, $title, $firstname, $lastname, $birthday, $email, $hash){
       $this->db->query("INSERT INTO Member(gender_id, member_title, member_firstname, member_lastname, member_birthday, member_email, member_password_hash, member_registration_date) VALUES(?, ?, ?, ?, ?, ?, ?, NOW())", array($salutation, $title, $firstname, $lastname, $birthday, $email, $hash));
    ...
    function get_account($p_command = "", $p_data = ""){
        ...
        switch ($v_type[0]) {
            case 'filter': 
                switch ($v_type[1]) {
                    case 'id': $query=$db_default->query("SELECT * FROM Member INNER JOIN MemberGroup USING(group_id) WHERE member_id = ?", array($p_data));
                    break;
                    ...
                    break;
                    case 'name': $query = $db_default->query("SELECT DISTINCT * FROM Member INNER JOIN MemberGroup USING(group_id) INNER JOIN Gender USING(gender_id) INNER JOIN Class USING(class_id) WHERE UPPER(member_firstname) LIKE CONCAT('%', UPPER(?), '%') OR UPPER(member_lastname) LIKE CONCAT('%', UPPER(?), '%')", array($p_data, $p_data));
                    break;
                    case 'company': $query = $db_default->query("SELECT DISTINCT * FROM Member INNER JOIN MemberGroup USING(group_id) INNER JOIN Gender USING(gender_id) INNER JOIN Class USING(class_id) WHERE company_name = ?", array($p_data));
                    break;
                    case 'class': $query = $db_default->query("SELECT DISTINCT * FROM Member INNER JOIN MemberGroup USING(group_id) INNER JOIN Gender USING(gender_id) INNER JOIN Class USING(class_id) WHERE class_name = ?", array($p_data));
                    break;

					case 'name_class_company': 
					$sqldata = preg_split("[:]", $p_data);
					$query=$db_default->query("SELECT DISTINCT * FROM Member INNER JOIN MemberGroup USING(group_id) INNER JOIN Gender USING(gender_id) INNER JOIN Class USING(class_id) WHERE (UPPER(member_firstname) LIKE '%'||(UPPER(?))||'%' OR UPPER(member_lastname) LIKE '%'||(UPPER(?))||'%') AND class_name = ?", array($sqldata[0], $sqldata[0], $sqldata[1], $sqldata[2]));
                }
            break;
            case 'group':
            ...
            case 'all':
                if($p_data != ""){
                    $query=$db_default->query("SELECT DISTINCT * FROM Member INNER JOIN MemberGroup USING(group_id) INNER JOIN Gender USING(gender_id) INNER JOIN Class USING(class_id) WHERE member_id = ?", array($p_data));
                }else{
                    $query=$db_default->query("SELECT DISTINCT * FROM Member INNER JOIN MemberGroup USING(group_id) INNER JOIN Gender USING(gender_id) INNER JOIN Class USING(class_id)  ORDER BY member_lastname ASC");
                }
            break;
        }
        return $query;
    ...
    function log_activity($p_account_id, $p_message, $p_ip_address, $p_browser, $p_os){
        $this->db->query("INSERT INTO MemberActivity (member_id, message_id, activity_ip_address, activity_data) VALUES(?, (SELECT message_id FROM MemberActivityMessage WHERE LOWER(message_title) = LOWER(?) LIMIT 1), ?, ?)", array($p_account_id, $p_message, $p_ip_address, $p_browser.';'.$p_os));
    }
	...
	function create_register_token($p_account_id = "", $p_token = ""){
        $this->db->query("INSERT INTO MemberToken(member_id, register_token, date_created) VALUES(?,?,NOW())", array($p_account_id, $p_token));
    }
	
	function get_register_token($p_email = ""){
		$query = $this->db->query("SELECT register_token, token_used FROM MemberToken INNER JOIN Member USING(member_id) WHERE member_email = ? AND SYSDATE() BETWEEN date_created AND (date_created + INTERVAL 1 HOUR)", array($p_email));
		return $query;
	}

    function use_register_token($p_email = "", $p_token = ""){
        $query = $this->db->query("UPDATE MemberToken SET token_used = 1 WHERE MemberToken.member_id = (SELECT Member.member_id FROM Member WHERE member_email = ?) AND register_token = ?", array($p_email, $p_token));
    }
}
