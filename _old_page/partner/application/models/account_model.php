...
class Account_Model extends CI_Model {
    ...
    function log_last_signin($p_account_id, $p_ip_address){
        $this->db->query("UPDATE Company SET company_last_ip = ? WHERE company_id= ?", array($p_ip_address, $p_account_id));
    }

    function log_activity($p_account_id, $p_message, $p_ip_address, $p_browser, $p_os){
        $this->db->query("INSERT INTO CompanyActivity (company_id, message_id, activity_ip_address, activity_data) VALUES(?, (SELECT message_id FROM CompanyActivityMessage WHERE LOWER(message_key_word) = LOWER(?) ), ?, ?)", array($p_account_id, $p_message, $p_ip_address, $p_browser.';'.$p_os));
    }
