...
class Account extends CI_Controller {
	...
	public function signin(){
		if(!$this->session->userdata('login')){
			...
			if(empty($username)){
				...
				if($this->Account_Model->get_account("filter:email", $username)->num_rows()==1){
					$v_account = $this->Account_Model->get_account("filter:email", $username)->row();
					if($this->Account_Model->is_locked($v_account->company_id)->num_rows() == 1){
						$data['error']=TRUE;
						$data['p_element_reset']=array("signin_password");
						$data['message'] = "Ihr Konto ist zurzeit gesperrt.".($this->Account_Model->is_locked($v_account->company_id)->row()->blocking_reason != "" ? "\n\n".$this->Account_Model->is_locked($v_account->member_id)->row()->blocking_reason."" : "")."\n\nBei Fragen wenden sie sich bitte an support@primus-romulus.net.";
					}else if($v_account->company_password_hash == $this->Account_Model->password_hash($username,$password)){
						...
						$this->Account_Model->delete_failed_login($v_account->company_id);
						$this->Account_Model->log_activity($v_account->company_id, "SIGN_IN_SUCCESS", $this->input->ip_address(), $this->agent->browser(), (!$this->agent->is_mobile() ? $this->agent->platform() : $this->agent->mobile()));
						$this->Account_Model->log_last_signin($v_account->company_id, $this->input->ip_address());
						$session_data = array(
							'login'=> TRUE,
						...
						$this->Account_Model->log_failed_login($v_account->company_id, $this->input->ip_address());
						$v_count_failed_login = $this->Account_Model->get_failed_login($v_account->company_id)->num_rows();
						switch ($v_count_failed_login) {
							...
							break;
							case 6:
								$this->Account_Model->lock_account($v_account->company_id, 3600, "Sie haben zu oft ein falsches Passwort eingegeben! Bitte versuchen Sie es später noch einmal.");
							...
							break;
							case 9:
								$this->Account_Model->lock_account($v_account->member_id, 86400, "Sie haben zu oft ein falsches Passwort eingegeben! Bitte versuchen Sie es später noch einmal.");
							...
							break;
							case 12:
								$this->Account_Model->lock_account($v_account->member_id, -1, "Ihr Konto wurde gesperrt, da Ihr Passwort zu oft falsch eingegeben wurde. Bitte wenden Sie sich an die unten angegebene E-Mail-Adresse.");
	...
	public function signup_password(){
		if(!$this->session->userdata('login')){
			...
			if(empty($p_password)){
				...
				if($this->Account_Model->get_account("filter:email", $p_email)->num_rows() == 1){
					$v_account = $this->Account_Model->get_account("filter:email", $p_email)->row();
					if($v_account->member_password_hash == $p_code && strtolower($v_account->group_name) == strtolower("Interested Person")){
						$this->Account_Model->update_password($v_account->member_id, $this->Account_Model->password_hash($v_account->member_email, $p_password));
						$this->Account_Model->set_group($v_account->member_id, "Member");
						$data['error']=FALSE;
	...
	public function update_account(){
		if($this->session->userdata("login")){
			...
			if($this->Location_Model->get_location(intval($this->session->userdata("account_id")), "all", $p_location)->num_rows() != 1 || $p_location == NULL){
				...
				$this->Account_Model->update_account(intval($this->session->userdata("account_id")), $p_location, $p_contact, $p_description);
				if($p_branche != NULL){
					$this->Account_Model->remove_branche(intval($this->session->userdata("account_id")));
					$p_branche = array_unique($p_branche);
					for ($i=0; $i < count($p_branche); $i++) {
						if($this->General_Model->get_branche($p_branche[$i])->num_rows() == 1 ){
							$this->Account_Model->add_branche(intval($this->session->userdata("account_id")), $p_branche[$i]);
						}						
					}
				}
				if($p_property_name != NULL && $p_property_text != NULL){
					if(count($p_property_name) == count($p_property_text)){
						$this->Account_Model->remove_properties(intval($this->session->userdata("account_id")));
						for ($i=0; $i < count($p_property_name) ; $i++) {
							if($p_property_name[$i] != "" && $p_property_text[$i] != ""){
								$this->Account_Model->add_property(intval($this->session->userdata("account_id")), $p_property_name[$i], $p_property_text[$i]);
							}
						}
					}
				}
				if($p_website_type != NULL && $p_website_name != NULL && $p_website_url != NULL){
					if(count($p_website_type) == count($p_website_name) && count($p_website_name) == count($p_website_url)){
						$this->Account_Model->remove_websites(intval($this->session->userdata("account_id")));
						for ($i=0; $i < count($p_website_name) ; $i++) {
							if($this->General_Model->get_website_type($p_website_type[$i])->num_rows() == 1 && $p_website_name[$i] != "" && $p_website_url[$i] != ""){
								$this->Account_Model->add_website(intval($this->session->userdata("account_id")), $p_website_name[$i], $p_website_type[$i], $p_website_url[$i]);
							}
						}
					}
				}
	...
	public function password_change(){
		if($this->session->userdata('login')){
			...
			if(empty($current)){
				...
				if($object_account->company_password_hash == $this->Account_Model->password_hash($object_account->company_email, $current) ){
					$this->Account_Model->log_activity($object_account->company_id, "PASSWORD_CHANGE", $this->input->ip_address(), $this->agent->browser(), (!$this->agent->is_mobile() ? $this->agent->platform() : $this->agent->mobile()));
					$this->Account_Model->update_password($this->session->userdata('account_id'), $this->Account_Model->password_hash($object_account->company_email, $new));
					$data['error']=FALSE;
	...
	public function remove_avatar(){
		if($this->session->userdata('login')){
			if($this->Account_Model->get_detail(intval($this->session->userdata('account_id')))->num_rows() == 1){
				...
				$this->Account_Model->remove_avatar(intval($this->session->userdata('account_id')));
				$data['error']=FALSE;
	...
	public function upload_logo(){
		...
		if(!isset($_FILES['file'])){
			...
			if ($v_img_width > 500) {
				...
				$this->Account_Model->delete_logo(intval($this->session->userdata("account_id")));
	...
	public function upload_banner(){
		...
		if(!isset($_FILES['file'])){
			...
			if ($v_img_width > 3000) {
				...
				$this->Account_Model->delete_banner(intval($this->session->userdata("account_id")));
	...
	public function recovery(){
		...
		}else if($this->Account_Model->get_code("filter:current:email", $v_email)->num_rows()==0){
			$data['error']=TRUE;
			$data['message']="Der Code ist leider ungültig, bitte versuchen Sie den Vorgang erneut.";
		}else{
			$object_account = $this->Account_Model->get_account("filter:email", $v_email)->row();
			$this->Account_Model->update_password($object_account->company_id, $this->Account_Model->password_hash($object_account->company_email, $v_password));
			$data['error']=FALSE;
	...
	public function support($type = ""){
		switch ($type) {
			case 'password_recovery':
				...
				}else if($this->Account_Model->get_account("filter:email",$email)->num_rows()==0){
					$data['error']=TRUE;
					$data['p_element_reset'] = array("recovery_password_email");
					$data['message'] = "Leider existiert zu dieser E-Mail-Adresse kein Konto.";
				}else{
					$object_account = $this->Account_Model->get_account("filter:email",$email)->row();
					if($this->Account_Model->is_locked($object_account->member_id)->num_rows()==1){
						$data['error']=TRUE;
						$data['p_element_reset'] = array("recovery_password_email");
						$data['message'] = "Ihr Konto ist zurzeit gesperrt deswegen können Sie Ihr Passwort nicht zurücksetzen.";
					}else if($this->Account_Model->get_code("last 24 hours",$object_account->member_id)->num_rows()==5){
						$data['error']=TRUE;
						$data['p_element_reset'] = array("recovery_password_email");
						...
						$this->Account_Model->create_recovery_code($object_account->member_id, $v_code);
