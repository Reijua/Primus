	<footer>
		<div class="footer-top">
			<div class="footer-wrapper">
				<div class="column-4">
					<div class="column-content">
						<h4>Absolventen</h4>
						<ul>
							<li><a href="http://primus-romulus.net/member/login">Anmelden</a></li>
							<li><a href="http://primus-romulus.net/member/register">Mitglied werden</a></li>
						</ul>
					</div>
				</div>
				<div class="column-4">
					<div class="column-content">
						<h4>Partner</h4>
						<ul>
							<li><a href="http://partner.primus-romulus.net/">Anmelden</a></li>
							<li><a href="http://primus-romulus.net/partner/information/">Partner werden</a></li>
						</ul>
					</div>
				</div>
				<div class="column-4">
					<div class="column-content">
						<h4>Social Network</h4>
						<ul>
							<li><a href="https://facebook.com/primusromulus/" target="_blank">Facebook</a></li>
							<li><a href="https://plus.google.com/101535514050835291864" rel="publisher" target="_blank">Google +</a></li>
							<li><a href="https://twitter.com/primusromulus/" target="_blank">Twitter</a></li>
						</ul>
					</div>
				</div>
				<div class="column-4">
					<div class="column-content">
						<h4>Kontakt</h4>
						<address>
							<strong>Primus Romulus</strong><br>
							<br>
							Hauptstraße 179<br>
							8141 Unterpremstätten<br>
							<br>
							<i class="icon-envelope"></i> <a href="mailto:office@primus-romulus.net">office@primus-romulus.net</a>
						</address>
					</div>
				</div>
			</div>
		</div>
		<div class="footer-bottom">
			<div class="footer-wrapper">
				<div class="column text-left" style="width:50%;">
					<div class="column-content" style="margin-top: 0; margin-bottom: 0;">&copy; 2015 Primus Romulus</div>
				</div>
				<div class="column text-right desktop-visible" style="width:50%;">
					<div class="column-content" style="margin-top: 0; margin-bottom: 0;">
						<a href="http://primus-romulus.net/site/legal/imprint/">Impressum</a> | <a href="http://primus-romulus.net/site/legal/tos/">Nutzungsbedingung</a> | <a href="http://primus-romulus.net/site/legal/gtc/">Datenschutzerklärung</a> | <a href="http://primus-romulus.net/site/legal/toc/">Cookie-Richtlinien</a>
					</div>
				</div>
				<div class="column text-right desktop-hidden" style="width:50%;">
					<div class="column-content" style="margin-top: 0; margin-bottom: 0;">
						<a href="http://primus-romulus.net/site/legal/">Rechtliches</a>
					</div>
				</div>
			</div>
			
		</div>
	</footer>
	<?php if(!$this->session->userdata("login")){ ?>
	<nav class="menu">
		<div class="menu-header">
			<ul class="option-bar">
				<li><i class="icon-remove"></i></li>
			</ul>
		</div>
		<div class="menu-wrapper" style="height: 746px;">
			<div class="menu-content">
				<div class="section">
					<h6>Allgemeines</h6>
					<ul>
						<li><a href="http://primus-romulus.net/">Startseite</a></li>
						<li><a href="http://primus-romulus.net/about/">Über uns</a></li>
					</ul>
				</div>
				<div class="section">
					<h6>Mitglieder</h6>
					<ul>
						<li><a href="http://primus-romulus.net/member/login">Anmelden</a></li>
						<li><a href="http://primus-romulus.net/member/register">Mitglied werden</a></li>
					</ul>
				</div>
				<div class="section">
					<h6>Partner</h6>
					<ul>
						<li><a href="http://partner.primus-romulus.net/">Anmelden</a></li>
						<li><a href="http://primus-romulus.net/partner/information/">Partner werden</a></li>
					</ul>
				</div>
				<div class="section">
					<hr style="margin:0;"/>
					<ul class="social-network">
						<li><a href="https://facebook.com/primusromulus/" target="_blank"><img src="<?php echo $cdn_url; ?>image/icon/social-network/facebook.png" alt="Facebook" /></a></li>
						<li><a href="https://plus.google.com/101535514050835291864" rel="publisher" target="_blank"><img src="<?php echo $cdn_url; ?>image/icon/social-network/google_plus.png" alt="Google +" /></a></li>
						<li><a href="https://twitter.com/primusromulus/" target="_blank"><img src="<?php echo $cdn_url; ?>image/icon/social-network/twitter.png" alt="Twitter" /></a></li>
					</ul>
				</div>
			</div>
		</div>
	</nav>
	<?php } else { ?>
	<nav class="menu">
		<div class="menu-header">
			<ul class="option-bar">
				<li><i class="icon-remove"></i></li>
			</ul>
		</div>
		<div class="menu-wrapper" style="height: 746px;">
			<div class="menu-content">
				<div class="section">
					<h6><?= $object_account->company_name ?></h6>
					<ul>
						<li><span class="modal link" data-title="Einstellungen" data-type="url" data-source="/ajax/modal/settings/">Einstellungen</span></li>
						<li><form methode="post" data-url="/ajax/account/logout/" data-redirect="/" data-type="confirm" data-text="Wollen Sie sich wirklich abmelden?"><span class="submit link">Abmelden</span></form></li>
					</ul>
				</div>
				<div class="section">
					<hr style="color:#CCCCCC;" />
					<ul class="social-network">
						<li><a href="https://facebook.com/primusromulus/" target="_blank"><img src="<?php echo $resource_url; ?>image/icon/social-network/facebook.png" alt="Facebook" /></a></li>
						<li><a href="https://plus.google.com/101535514050835291864" rel="publisher" target="_blank"><img src="<?php echo $resource_url; ?>image/icon/social-network/google_plus.png" alt="Google +" /></a></li>
						<li><a href="https://twitter.com/primusromulus/" target="_blank"><img src="<?php echo $resource_url; ?>image/icon/social-network/twitter.png" alt="Twitter" /></a></li>
					</ul>
				</div>
			</div>
		</div>
	</nav>
	<?php } ?>
	</body>
</html>