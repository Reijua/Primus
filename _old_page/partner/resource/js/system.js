...
(function($) {
	...
	var system = {
		...
		sendRequest : function(options){
			...
			$.ajax({
				...
				error: function(){
					alert("Irgendwas ist hier komisch!");
	...
	jQuery.fn.initDynamicField = function(options){
		...
		var v_content = $(this).children(".dynamic-content");
	...
	jQuery.fn.initUI = function(options){
		var v_nav_width = 10;
		var v_nav_list = $(".nav-holder").children("nav").children("ul");
		var v_application_content = $("#application-content");

		v_nav_list.children().each(function(){
			v_nav_width += $(this).outerWidth(true);
		});

		v_nav_list.css({"width" : v_nav_width+"px"});
		v_application_content.html(system.loadingAnimation());
		$.post( "/ajax/application/get_content/", function(data) {
	  		v_application_content.html(data);
		});
