...
class Account extends CI_Controller {
		...
		if(!$this->session->userdata('login')){
			...
			if(empty($username)){
				...
				if($this->Account_Model->get_account("filter:email", $username)->num_rows()==1){
					...
					if($this->Account_Model->is_locked($v_account->member_id)->num_rows() == 1){
						$data['error']=TRUE;
						$data['p_element_reset']=array("signin_password");
						$data['message'] = "Ihr Konto ist zurzeit gesperrt.".($this->Account_Model->is_locked($v_account->member_id)->row()->blocking_reason != "" ? "\n\n".$this->Account_Model->is_locked($v_account->member_id)->row()->blocking_reason."" : "")."\n\nBei Fragen wenden sie sich bitte an support@primus-romulus.net.";
					}else if($v_account->member_password_hash == $this->Account_Model->password_hash($username,$password)){
						...
						$this->Account_Model->delete_failed_login($v_account->member_id);
						$this->Account_Model->log_activity($v_account->member_id, "login success", $this->input->ip_address(), $this->agent->browser(), (!$this->agent->is_mobile() ? $this->agent->platform() : $this->agent->mobile()));
						$this->Account_Model->log_last_login($v_account->member_id, $this->input->ip_address());
						$session_data = array(
							'login'=> TRUE,
						...
						$this->Account_Model->log_failed_login($v_account->member_id, $this->input->ip_address());
						$v_count_failed_login = $this->Account_Model->get_failed_login($v_account->member_id)->num_rows();
						switch ($v_count_failed_login) {
							...
							break;
							case 6:
								$this->Account_Model->lock_account($v_account->member_id, 3600, "Sie haben zu oft ein falsches Passwort eingegeben! Bitte versuchen Sie es später noch einmal.");
	...
	public function signup(){
		if(!$this->session->userdata('login')){
			...
			if($this->Account_Model->check_sign_up_blocking($this->input->ip_address())->num_rows()==1){
				$data['error'] = TRUE;
	        	...
	        	if($this->Account_Model->get_account("filter:email", $email)->num_rows() == 0){
	        		$birthday = $birthday_year.'-'.$birthday_month.'-'.$birthday_day;
	        		$v_id = $this->Account_Model->create_account($salutation, $title, $firstname, $lastname, $birthday, $email);
	        		$this->Account_Model->lock_account($v_id, -1, "Ihre eingegeben Daten werden zur Zeit überprüft. Wir melden uns sobald wir mit diesem Vorgang fertig sind.");
	        		$this->Account_Model->create_sign_up_blocking($this->input->ip_address());
	        		$data['error'] = FALSE;
	...
	public function password_change(){
		if($this->session->userdata('login')){
			...
			if(empty($current)){
				...
				$obj_account = $this->Account_Model->get_account("all", intval($this->session->userdata('account_id')))->row();
				if($obj_account->member_password_hash == $this->Account_Model->password_hash($obj_account->member_email, $current) ){
					$this->Account_Model->log_activity($obj_account->member_id, "password change", $this->input->ip_address(), $this->agent->browser(), (!$this->agent->is_mobile() ? $this->agent->platform() : $this->agent->mobile()));
					$this->Account_Model->update_password($this->session->userdata('account_id'), $this->Account_Model->password_hash($obj_account->member_email, $new));
					$data['error']=FALSE;
	...
	public function upload_avatar(){
		...
		if(!isset($_FILES['file'])){
			...
			if($v_img_width != $v_img_height){
				...
				$this->Account_Model->upload_avatar(intval($this->session->userdata('account_id')),$base64url);
				$data['error']=FALSE;
	...
	public function recovery(){
		...
		}else if($this->Account_Model->get_code("current", $v_email, $v_code)->num_rows()!=1){
			$data['error']=TRUE;
			$data['message']="Der Code ist leider ungültig, bitte versuchen Sie den Vorgang erneut.";
		}else{
			$object_account = $this->Account_Model->get_code("current", $v_email, $v_code)->row();
			$this->Account_Model->update_password($object_account->member_id, $this->Account_Model->password_hash($object_account->member_email, $v_password));
			$data['error']=FALSE;
